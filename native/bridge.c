...
static int32_t spy_ssz_child(spy_ssz_object$NativeSszObject *obj,
                             int32_t node, int32_t child) {
    spy_ssz_object$SszNode current = obj->nodes.p[node];
    return obj->edges.p[current.first_edge + child];
}

static int32_t spy_ssz_fixed_list_size(spy_ssz_object$NativeSszObject *obj,
                                       int32_t node) {
